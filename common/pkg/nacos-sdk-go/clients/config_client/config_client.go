...
import (
	...
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/clients/cache"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/clients/nacos_client"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/constant"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/http_agent"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/logger"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/nacos_error"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/model"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/util"
...
func init() {
	...
	go delayScheduler(time.NewTimer(100*time.Millisecond), 1*time.Second, "root", listenConfigExecutor())
}
...
func (client *ConfigClient) sync() (clientConfig constant.ClientConfig, serverConfigs []constant.ServerConfig, agent http_agent.IHttpAgent, err error) {
	...
	return func() error {
		...
		if taskCount > currentTaskCount {
			for i := currentTaskCount; i < taskCount; i++ {
				...
				go delayScheduler(time.NewTimer(100*time.Millisecond), 1*time.Second, strconv.Itoa(i), longPulling(i))
			}
...
func longPulling(taskId int) func() error {
	return func() error {
		...
		for _, key := range cacheMap.Keys() {
			var (
				cData              cacheData
				listeningConfigs   string
				initializationList []cacheData
			)
			...
			if value, ok := cacheMap.Get(key); ok {
				...
				temp[cData.tenant] = append(temp[cData.tenant], map[string]interface{}{
					"cData":              cData,
		...
		for nsId, configs := range temp {
			...
			for _, config := range configs {
				cData = config["cData"].(cacheData)
				client = cData.configClient
				listeningConfigs += config["listeningConfigs"].(string)
				initializationList = config["initializationList"].([]cacheData)
			}
			...
			if len(listeningConfigs) > 0 {
				...
				if len(strings.ToLower(strings.Trim(changed, " "))) == 0 {
					logger.Infof("[client.ListenConfig] namespace: %s no change", nsId)
				} else {
					logger.Infof("[client.ListenConfig] namespace: %s config changed: %s", nsId, changed)
...
func (client *ConfigClient) PublishAggr(param vo.ConfigParam) (published bool, err error) {
	if len(param.DataId) <= 0 {
...
func (client *ConfigClient) RemoveAggr(param vo.ConfigParam) (published bool, err error) {
	if len(param.DataId) <= 0 {
