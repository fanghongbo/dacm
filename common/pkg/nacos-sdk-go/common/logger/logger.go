...
import (
	...
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/file"
...
func getWriter(outputPath string, rotateTime string, maxAge int64) (writer io.Writer, err error) {
	...
	writer, err = rotatelogs.New(filepath.Join(outputPath, "dacm.log-%Y%m%d%H%M"),
		rotatelogs.WithRotationTime(rotateDuration), rotatelogs.WithMaxAge(time.Duration(maxAge)*rotateDuration),
		rotatelogs.WithLinkName(filepath.Join(outputPath, "dacm.log")))
	return
}
