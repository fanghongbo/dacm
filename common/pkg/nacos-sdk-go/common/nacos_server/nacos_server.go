...
import (
	...
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/common/security"
	"github.com/fanghongbo/dacm/common/pkg/nacos-sdk-go/inner/uuid"
...
func (server *NacosServer) refreshServerSrvIfNeed() {
	...
	//logger.Debugf("[client.ServerList] http nacos server list: <%s>", result)
	...
	for _, line := range list {
		if line != "" {
			...
			if len(splitLine) == 2 {
				...
				if err != nil {
					logger.Errorf("[client.ServerList] get port from server:<%s>  error: <%+v>", line, err)
					continue
				}
			}
	...
	if len(servers) > 0 {
		if !reflect.DeepEqual(server.serverList, servers) {
			server.Lock()
			logger.Debugf("[client.ServerList] server list is updated, old: <%v>,new:<%v>", server.serverList, servers)
